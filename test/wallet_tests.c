/**********************************************************************
 * Copyright (c) 2015 Jonas Schnelli                                  *
 * Distributed under the MIT software license, see the accompanying   *
 * file COPYING or http://www.opensource.org/licenses/mit-license.php.*
 **********************************************************************/

#include <iop/wallet.h>
#include <iop/base58.h>

#include <logdb/logdb.h>

#include "utest.h"
#include <iop/utils.h>
#include <unistd.h>

static const char *wallettmpfile = "/tmp/dummy";

void test_wallet()
{
    unlink(wallettmpfile);
    iop_wallet *wallet = iop_wallet_new(&iop_chainparams_main);
    int error;
    iop_bool created;
    u_assert_int_eq(iop_wallet_load(wallet, wallettmpfile, &error, &created), true);

    char *xpriv = "xprv9uHRZZhk6KAJC1avXpDAp4MDc3sQKNxDiPvvkX8Br5ngLNv1TxvUxt4cV1rGL5hj6KCesnDYUhd7oWgT11eZG7XnxHrnYeSvkzY7d2bhkJ7";

    iop_hdnode node;
    iop_wallet_hdnode *node2, *node3;
    iop_bool suc = iop_hdnode_deserialize(xpriv, &iop_chainparams_main, &node);
    u_assert_int_eq(suc, 1);
    iop_wallet_set_master_key_copy(wallet, &node);

    node2 = iop_wallet_next_key(wallet);

    iop_wallet_free(wallet);

    wallet = iop_wallet_new(&iop_chainparams_main);
    u_assert_int_eq(iop_wallet_load(wallet, wallettmpfile, &error, &created), true);
    node3 = iop_wallet_next_key(wallet);

    //should not be equal because we autoincrementing child index
    u_assert_int_eq(memcmp(node2->hdnode->private_key, node3->hdnode->private_key, sizeof(node2->hdnode->private_key)) != 0, 1);
    iop_wallet_hdnode_free(node3);

    //force to regenerate child 0
    wallet->next_childindex = 0;
    node3 = iop_wallet_next_key(wallet);
    iop_wallet_hdnode_free(node3);
    wallet->next_childindex = 0;
    node3 = iop_wallet_next_key(wallet);

    //now it should be equal
    u_assert_int_eq(memcmp(node2->hdnode->private_key, node3->hdnode->private_key, sizeof(node2->hdnode->private_key)), 0);

    uint160 hash160;

    vector *addrs = vector_new(1, free);
    iop_wallet_get_addresses(wallet, addrs);
    u_assert_int_eq(addrs->len, 4);
    u_assert_int_eq(strcmp(addrs->data[3],"1JQheacLPdM5ySCkrZkV66G2ApAXe1mqLj"), 0)
    u_assert_int_eq(strcmp(addrs->data[0],"1LZaBnH11M2yN5ZNiK67yUbaspfX6XKmRr"), 0)

    vector_free(addrs, true);
    iop_wallet_flush(wallet);
    iop_wallet_free(wallet);
    iop_wallet_hdnode_free(node2);
    iop_wallet_hdnode_free(node3);

    wallet = iop_wallet_new(&iop_chainparams_main);
    u_assert_int_eq(iop_wallet_load(wallet, wallettmpfile, &error, &created), true);
    addrs = vector_new(1, free);
    iop_wallet_get_addresses(wallet, addrs);

    u_assert_int_eq(addrs->len, 4);
    u_assert_int_eq(strcmp(addrs->data[3],"1JQheacLPdM5ySCkrZkV66G2ApAXe1mqLj"), 0)
    u_assert_int_eq(strcmp(addrs->data[0],"1LZaBnH11M2yN5ZNiK67yUbaspfX6XKmRr"), 0)

    // get a hdnode and compare
    iop_wallet_hdnode *checknode = iop_wallet_find_hdnode_byaddr(wallet, addrs->data[0]);

    hash160[0] = wallet->chain->b58prefix_pubkey_address;
    iop_hdnode_get_hash160(checknode->hdnode, hash160+1);

    size_t addrsize = 98;
    char addr[addrsize];
    iop_base58_encode_check(hash160, sizeof(uint160)+1, addr, addrsize);
    u_assert_int_eq(strcmp(addr, addrs->data[0]), 0);
    vector_free(addrs, true);

    // add some txes
    static const char *hextx_coinbase = "";

    static const char *hextx_ntx = "";

    // form coinbase tx
    uint8_t tx_data[strlen(hextx_coinbase) / 2];
    int outlen ;
    utils_hex_to_bin(hextx_coinbase, tx_data, strlen(hextx_coinbase), &outlen);
    iop_wtx* wtx = iop_wallet_wtx_new();
    iop_tx_deserialize(tx_data, outlen, wtx->tx, NULL);

    // add coinbase tx
    wtx->height = 0;
    iop_wallet_add_wtx_move(wallet, wtx);

    uint64_t amount = iop_wallet_wtx_get_credit(wallet, wtx);
    u_assert_uint32_eq(amount, 0);
    wallet->bestblockheight = 200;
    amount = iop_wallet_wtx_get_credit(wallet, wtx);

    u_assert_uint32_eq(amount, 2504815547);
    iop_wallet_wtx_free(wtx);

    // form normal tx
    uint8_t tx_data_n[strlen(hextx_ntx) / 2];
    utils_hex_to_bin(hextx_ntx, tx_data_n, strlen(hextx_ntx), &outlen);
    wtx = iop_wallet_wtx_new();
    iop_tx_deserialize(tx_data_n, outlen, wtx->tx, NULL);
    
    // add normal tx
    wtx->height = 0;
    iop_wallet_add_wtx_move(wallet, wtx);

    amount = iop_wallet_wtx_get_credit(wallet, wtx);
    iop_wallet_wtx_free(wtx);

    u_assert_uint32_eq(amount, 13326620644);

    iop_wallet_flush(wallet);
    iop_wallet_free(wallet);

    wallet = iop_wallet_new(&iop_chainparams_main);
    u_assert_int_eq(iop_wallet_load(wallet, wallettmpfile, &error, &created), true);

    amount = iop_wallet_get_balance(wallet);

    vector *unspents = vector_new(10, (void (*)(void*))iop_wallet_output_free);
    iop_wallet_get_unspent(wallet, unspents);

    amount = iop_wallet_get_balance(wallet);

    unsigned int i;
    unsigned int found = 0;
    for (i = 0; i < unspents->len; i++)
    {
        iop_output *output = unspents->data[i];
        uint256 hash;
        iop_tx_hash(output->wtx->tx, hash);
        char str[65];
        utils_bin_to_hex(hash, sizeof(hash), str);
        utils_reverse_hex(str, 64);
        if (strcmp(str, "963b8b8e2d2025b64fd8144557604e98d2fa67a5386f8a06597d810f27ab60d7") == 0)
            found++;
        if (strcmp(str, "b99c4c532643a376c440b3cc612ec2fd96c15d1f50a6c40b112e4fd0c880d661") == 0)
            found++;
    }
    vector_free(unspents, true);
    u_assert_int_eq(found, 2);

    amount = iop_wallet_get_balance(wallet);
    u_assert_uint32_eq(amount, 13326620644);
    wallet->bestblockheight = 200;
    amount = iop_wallet_get_balance(wallet);
    u_assert_uint32_eq(amount, 13326620644+2504815547);

    iop_wallet_free(wallet);
}
