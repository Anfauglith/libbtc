...
void test_protocol()
{
    ...
    iop_p2p_address ipv6Test;
    iop_p2p_address_init(&ipv6Test);
    iop_addr_to_p2paddr((struct sockaddr *)&test_sa6, &ipv6Test);
    iop_p2paddr_to_addr(&ipv6Test, (struct sockaddr *)&test_sa6_check);
    memset(&i6buf, 0, 1024);
    ...
    iop_addr_to_p2paddr((struct sockaddr *)&test_sa, &toAddr);
    iop_p2paddr_to_addr(&toAddr, (struct sockaddr *)&test_sa_check);
    ...
    iop_p2p_inv_msg inv_msg, inv_msg_check;
    memset(&inv_msg, 0, sizeof(inv_msg));

    uint256 hash = {0};

    iop_p2p_msg_inv_init(&inv_msg, 1, hash);
    iop_p2p_msg_inv_ser(&inv_msg, inv_msg_cstr);

    struct const_buffer buf_inv = {inv_msg_cstr->str, inv_msg_cstr->len};
    u_assert_int_eq(iop_p2p_msg_inv_deser(&inv_msg_check, &buf_inv), true);
    ...
    iop_p2p_msg_version_init(&version_msg, &fromAddr, &toAddr, "client", false);
    iop_p2p_msg_version_ser(&version_msg, version_msg_cstr);

    /* create p2p message */
    cstring *p2p_msg = iop_p2p_message_new((unsigned const char *)&iop_chainparams_main.netmagic, IOP_MSG_VERSION, version_msg_cstr->str, version_msg_cstr->len);

    struct const_buffer buf = {p2p_msg->str, p2p_msg->len};
    iop_p2p_msg_hdr hdr;
    iop_p2p_deser_msghdr(&hdr, &buf);

    u_assert_mem_eq(hdr.netmagic, &iop_chainparams_main.netmagic, 4);
    u_assert_str_eq(hdr.command, IOP_MSG_VERSION);
    ...
    u_assert_int_eq(iop_p2p_msg_version_deser(&v_msg_check, &buf), true);

    u_assert_int_eq(v_msg_check.version, IOP_PROTOCOL_VERSION);
    ...
    p2p_msg = iop_p2p_message_new((unsigned const char *)&iop_chainparams_main.netmagic, IOP_MSG_GETHEADERS, getheader_msg->str, getheader_msg->len);


    buf.p = p2p_msg->str;
    buf.len = p2p_msg->len;
    iop_p2p_deser_msghdr(&hdr, &buf);
    u_assert_str_eq(hdr.command, IOP_MSG_GETHEADERS);
    u_assert_int_eq(hdr.data_len, getheader_msg->len);


    uint256 hashstop_check;
    vector *blocklocators_check = vector_new(1, free);
    iop_p2p_deser_msg_getheaders(blocklocators_check, hashstop_check, &buf);
