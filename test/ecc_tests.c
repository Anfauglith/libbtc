...
void test_ecc()
{
    ...
    while (iop_ecc_verify_privatekey(r_buf) == 0) {
        u_assert_int_eq(iop_random_bytes(r_buf, 32, 0), true);
    }

    memset(r_buf, 0xFF, 32);
    u_assert_int_eq(iop_ecc_verify_privatekey(r_buf), 0); //secp256k1 overflow
    ...
    u_assert_int_eq(iop_ecc_verify_pubkey(pub_key33, 1), 1);
    u_assert_int_eq(iop_ecc_verify_pubkey(pub_key65, 0), 1);

    u_assert_int_eq(iop_ecc_verify_pubkey(pub_key33_invalid, 1), 0);
    u_assert_int_eq(iop_ecc_verify_pubkey(pub_key65_invalid, 0), 0);
    ...
    size_t sigderlen = 74;
    u_assert_int_eq(iop_ecc_der_to_compact(sig, outlen, sigcomp), true);
    u_assert_int_eq(iop_ecc_compact_to_der_normalized(sigcomp, sigder, &sigderlen),  true);
