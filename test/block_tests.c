...
void test_block_header()
{
    ...
    for (i = 0; i < (sizeof(block_header_tests) / sizeof(block_header_tests[0])); i++) {
        ...
        iop_block_header* header = iop_block_header_new();
        struct const_buffer buf = {header_data, 80};
        iop_block_header_deserialize(header, &buf);

        // Check the copies are the same
        iop_block_header* header_copy = iop_block_header_new();
        iop_block_header_copy(header_copy, header);
        assert(memcmp(header_copy, header, sizeof(*header_copy)) == 0);

        // Check the serialized form matches
        iop_block_header_serialize(serialized, header);
        utils_bin_to_hex((unsigned char*) serialized->str, serialized->len, hexbuf);
        assert(memcmp(hexbuf, test->hexheader, 160) == 0);

        // Check the block hash
        uint256 blockhash;
        iop_block_header_hash(header, blockhash);

        utils_bin_to_hex(blockhash, IOP_HASH_LENGTH, hexbuf);
        utils_reverse_hex(hexbuf, IOP_HASH_LENGTH*2);
        assert(memcmp(hexbuf, test->hexhash, IOP_HASH_LENGTH*2) == 0);
        ...
        iop_block_header_free(header);
        iop_block_header_free(header_copy);
        cstr_free(serialized, true);
    }


    /* blockheader */
    iop_block_header bheader, bheaderprev, bheadercheck;
    ...
    char *prevblock_hex = iop_malloc(strlen(prevblock_hex_o)+1);
    ...
    prevblock_hex = iop_malloc(strlen(prevblock_hex_o)+1);
    memcpy(prevblock_hex, prevblock_hex_o, strlen(prevblock_hex_o));
    utils_reverse_hex(prevblock_hex, 64);
    outlen = 0;
    ...
    merkleroot_hex = iop_malloc(strlen(merkleroot_hex_o)+1);
    memcpy(merkleroot_hex, merkleroot_hex_o, strlen(merkleroot_hex_o));
    utils_reverse_hex(merkleroot_hex, 64);
    outlen = 0;
    ...
    iop_block_header_serialize(blockheader_ser, &bheader);
    ...
    uint256 checkhash;
    iop_block_header_hash(&bheader, (uint8_t *)&checkhash);
    ...
    iop_block_header_deserialize(&bheadercheck, &buf);
    u_assert_mem_eq(&bheader, &bheadercheck, sizeof(bheadercheck));
    cstr_free(blockheader_ser, true);


    iop_block_header_hash(&bheaderprev, (uint8_t *)&checkhash);
