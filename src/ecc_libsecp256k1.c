...
void iop_ecc_start(void)
{
    ...
    uint8_t seed[32];
    assert(iop_random_bytes(seed, 32, 0));
    int ret = secp256k1_context_randomize(secp256k1_ctx, seed);
    assert(ret);
}


void iop_ecc_stop(void)
{
...
void iop_ecc_get_pubkey(const uint8_t* private_key, uint8_t* public_key, size_t* in_outlen, iop_bool compressed)
{
...
iop_bool iop_ecc_private_key_tweak_add(uint8_t* private_key, const uint8_t* tweak)
{
    assert(secp256k1_ctx);
    return secp256k1_ec_privkey_tweak_add(secp256k1_ctx, (unsigned char*)private_key, (const unsigned char*)tweak);
}

iop_bool iop_ecc_public_key_tweak_add(uint8_t* public_key_inout, const uint8_t* tweak)
{
    size_t out = IOP_ECKEY_COMPRESSED_LENGTH;
    secp256k1_pubkey pubkey;
...
iop_bool iop_ecc_verify_privatekey(const uint8_t* private_key)
{
    assert(secp256k1_ctx);
    return secp256k1_ec_seckey_verify(secp256k1_ctx, (const unsigned char*)private_key);
}

iop_bool iop_ecc_verify_pubkey(const uint8_t* public_key, iop_bool compressed)
{
    secp256k1_pubkey pubkey;
...
iop_bool iop_ecc_sign(const uint8_t* private_key, const uint256 hash, unsigned char* sigder, size_t* outlen)
{
    assert(secp256k1_ctx);
...
iop_bool iop_ecc_sign_compact(const uint8_t* private_key, const uint256 hash, unsigned char* sigcomp, size_t* outlen)
{
    assert(secp256k1_ctx);
...
iop_bool iop_ecc_sign_compact_recoverable(const uint8_t* private_key, const uint256 hash, unsigned char* sigrec, size_t* outlen, int *recid)
{
    assert(secp256k1_ctx);
...
iop_bool iop_ecc_recover_pubkey(const unsigned char* sigrec, const uint256 hash, const int recid, uint8_t* public_key, size_t *outlen)
{
    assert(secp256k1_ctx);
...
iop_bool iop_ecc_verify_sig(const uint8_t* public_key, iop_bool compressed, const uint256 hash, unsigned char* sigder, size_t siglen)
{
    assert(secp256k1_ctx);
...
iop_bool iop_ecc_compact_to_der_normalized(unsigned char* sigcomp_in, unsigned char* sigder_out, size_t* sigder_len_out)
{
    assert(secp256k1_ctx);
...
iop_bool iop_ecc_der_to_compact(unsigned char* sigder_in, size_t sigder_len, unsigned char* sigcomp_out)
{
    assert(secp256k1_ctx);
